...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)),os.path.pardir))  # 添加路径
from web import app  # 导入app包


db = SQLAlchemy(app.app)   # 实例化SQLAlchemy对象


class Account(db.Model):  # Account(账单的意思) 用户实体,保存用户注册手机号,用户名,密码,注册时间
    id = db.Column(db.Integer,primary_key=True)  # id,int类型
    username = db.Column(db.String(50))  # 用户名
    password = db.Column(db.String(255))  # 用户密码
    name = db.Column(db.String(20))  # 名字
    telephone = db.Column(db.Strint(50), unique=True)  # 电话号, unique 唯一, 不允许多人使用相同电话号
    role = db.Column(db.Integer)  # 角色
    flag_telephone = db.Column(db.Integer)  # 标志电话号码
    checkcode = db.Column(db.String(50))  # 检查编码
    source = db.Column(db.string(20))  # 源
    dtcreate = db.Column(db.DateTime)  # 创建时间
